...
!*/
        ...
        return (
            ...
            _createClass(Bonzi, [
                ...
                },
                {
                    key: "updateName",
                    value: function () {
                        && this.$nameIcon.html("<i class='fas fa-hammer' title='Godmode' style='color: #9C413E;' />")
    ...
    function bzSetup() {
        window.adsShown || ((window.adsShown = !0), showAds()),
            ...
            socket.on("room", function (data) {
                ...
                // BonziTV: load room-specific background video for default rooms.
                // Uses a queue so it works whether the YT IFrame API is ready yet or not.
                window._bonziTVRoom = data.room;
                window._bonziTVStart();
            }),
            ...
            socket.on("updateAll", function (data) {
                $("#page_login").hide();
                // Purge any bonzis whose guids are no longer in the incoming usersPublic.
                // Without this, stale bonzis from dropped connections (e.g. after idle/reconnect)
                // accumulate because no 'leave' event is ever received for them.
                var incomingGuids = Object.keys(data.usersPublic);
                Object.keys(usersPublic).forEach(function(oldGuid) {
                    if (incomingGuids.indexOf(oldGuid) === -1) {
                        var stale = bonzis[oldGuid];
                        if (stale) { try { stale.deconstruct(); } catch(e) {} delete bonzis[oldGuid]; }
                        delete usersPublic[oldGuid];
                    }
                });
                usersPublic = data.usersPublic; usersUpdate(); BonziHandler.bonzisCheck();
            }),
            ...
            socket.on("room", (function(e) {
                if ("news" == e.room) {
                    $("#bonzi_tv").html("<div id='bonzi_tv_player' style='width:100%;height:100%;'></div>");
                    ...
                    new YT.Player("bonzi_tv_player", {
                        ...
                        width: "100%",
                        videoId: "3RpxJT5w7Tc",
                        playerVars: { autoplay: 1, modestbranding: 1, controls: 0, loop: 1 }
                    });
    ...
    // BonziTV helpers — defined here so they're inside the same scope as socket/jQuery.
    // _bonziTVRoom holds the pending room name set by socket.on("room").
    // _bonziTVStart() creates the player; called either immediately (if YT already ready)
    // or deferred via onYouTubeIframeAPIReady if the API script hasn't finished loading yet.
    var _bonziTVVideos = {
        "news":    { videoId: "9Auq9mYxFEE", loop: false },
        "why":     { videoId: "3RpxJT5w7Tc", loop: true  },
        "area_51": { videoId: "TRc85qoNo6w", loop: true  },
        "poland":  { videoId: "rRPQs_kM_nw", loop: true  }
    };
    window._bonziTVRoom = null;
    window._bonziTVStart = function() {
        var room = window._bonziTVRoom;
        var cfg  = _bonziTVVideos[room];
        if (!cfg) { $("#bonzi_tv").html(""); return; }
        // Defer if YT IFrame API not ready yet
        if (typeof YT === "undefined" || typeof YT.Player === "undefined") return;
        $("#bonzi_tv").html("<div id='bonzi_tv_player' style='width:100%;height:100%;'></div>");
        var playerVars = { autoplay: 1, modestbranding: 1, controls: 0 };
        if (cfg.loop) { playerVars.loop = 1; playerVars.playlist = cfg.videoId; }
        new YT.Player("bonzi_tv_player", {
    ...
    // Called by the YT IFrame API script when it finishes loading.
    // If a room was already received before the API was ready, start the player now.
