        ...
        return (
            ...
            _createClass(Bonzi, [
                ...
                },
                {
                    key: "updateName",
                    value: function () {
                            this.userPublic && (this.userPublic.runlevel >= 3 || (this.userPublic.flags && this.userPublic.flags.admin)) ? this.$nameIcon.html("<i class='fas fa-gavel' style='color:#ff6b35'></i>") : this.$nameIcon.html("");
                ...
                },
                {
                    ...
                    value: function (vid) {
                        if (!this.mute && vid) {
                            ...
                            var match = vid.match(regExp);
                            if (match && match[7] && match[7].length == 11) {
                                videoId = match[7];
                            }

                            // Clear any existing content first
                            self.$dialogCont.empty();
                            self.$dialogCont.html("<iframe width='480' height='270' src='https://www.youtube.com/embed/" + videoId + "?autoplay=1&modestbranding=1&playsinline=0&showinfo=0&enablejsapi=1&color=purple&theme=dark' frameborder='0' allow='autoplay; encrypted-media' allowfullscreen style='border-radius:7px;'></iframe>");
                ...
                },
                {
                    ...
                    value: function (a) {
                        if (!this.mute) {
                            ...
                            this.$dialog.addClass("video");
                            this.$dialog.addClass("autosize");

                            var mw = Math.floor(window.innerWidth - this.x);
                            if (mw > 400) mw = 400;
                            mw -= 20;

                            this.$dialogCont.html("<video width='" + mw + "' controls autoplay><source src='" + a.url + "' type='video/mp4'></video>");
    ...
    function bzSetup() {
        window.adsShown || ((window.adsShown = !0), showAds()),
            ...
            socket.on("talk", function (data) {
                ...
                b && (b.cancel(!0), b.runSingleEvent([{ type: "text", text: data.text, audio: data.audio }]));
            }),
...
setInterval(function() {
    var specialRooms = [
        { _id: "area_51", name: "area_51", users: 0, locked: false },
        { _id: "default", name: "default", users: 0, locked: false },
        { _id: "poland", name: "poland", users: 0, locked: false },
        { _id: "news", name: "news", users: 0, locked: false },
        { _id: "why", name: "why", users: 0, locked: false }
    ];

    var scope = angular.element(".side-bar-content").scope();
    if (scope && scope.sbd && scope.sbd.rooms) {
        specialRooms.forEach(function(room) {
            var exists = scope.sbd.rooms.some(function(r) { return r._id === room._id; });
            if (!exists) {
                scope.sbd.rooms.unshift(room);
            }
        });
        if (scope.sbd.rooms.length > 0) {
            scope.$apply();
        }
    }
}, 3000);

$(document).on("click", ".room-directory-list .rdl-inner .fa-sign-in-alt", function() {
    var roomId = $(this).closest(".rdl-inner").find(".name").text().trim();
